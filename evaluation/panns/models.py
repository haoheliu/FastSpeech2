...
class Cnn14(nn.Module):
    def __init__(self,features_list, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num):
        ...
        if(sample_rate == 16000):
            state_dict = torch.load("/mnt/fast/nobackup/users/hl01486/projects/general_audio_generation/conditional_transfer/FastSpeech2/evaluation/logs/panns/Cnn14_16k_mAP=0.438.pth")
            self.load_state_dict(state_dict["model"])
        elif(sample_rate == 32000):
            state_dict = torch.load("/mnt/fast/nobackup/users/hl01486/projects/general_audio_generation/conditional_transfer/FastSpeech2/evaluation/logs/panns/Cnn14_mAP=0.431.pth")
            self.load_state_dict(state_dict["model"])
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x = self.conv_block1(x, pool_size=(2, 2), pool_type='avg'); embeddings.append(x)
        x = F.dropout(x, p=0.2, training=self.training)
        x = self.conv_block2(x, pool_size=(2, 2), pool_type='avg'); embeddings.append(x)
        x = F.dropout(x, p=0.2, training=self.training)
        x = self.conv_block3(x, pool_size=(2, 2), pool_type='avg'); embeddings.append(x)
        x = F.dropout(x, p=0.2, training=self.training)
        x = self.conv_block4(x, pool_size=(2, 2), pool_type='avg'); embeddings.append(x)
        x = F.dropout(x, p=0.2, training=self.training)
        x = self.conv_block5(x, pool_size=(2, 2), pool_type='avg'); embeddings.append(x)
        x = F.dropout(x, p=0.2, training=self.training)
        x = self.conv_block6(x, pool_size=(1, 1), pool_type='avg'); embeddings.append(x)
        x = F.dropout(x, p=0.2, training=self.training)
